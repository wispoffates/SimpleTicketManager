...
public class ClaimCommand implements ICommand {
    ...
    public ClaimCommand(SimpleTicketManager pl, TicketManager m) {
        this.plugin = pl;
        this.manager = m;
    ...
    public boolean onCommand(Player player, Command command, String label, String[] args) {
        ...
        if (args.length > 1) {
            ...
            manager.setTicketAssigned(player, player.getUniqueId().toString(), id);
        } else {
            player.sendMessage(prefix + ChatColor.translateAlternateColorCodes('&', plugin.messageData.get("ticketClaimError")));
        }
